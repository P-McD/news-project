...
const fetchTopics = () => {
  let fetchTopicsStr = `
    SELECT * FROM topics;
    `;
  return db.query(fetchTopicsStr).then(({ rows }) => {
    if (rows.length > 0) {
      return rows;
    } else {
...
const fetchArticles = (topic, sort_by = "created_at", order = "DESC") => {
  ...
  if (topic) {
    let topicQueryValue = topic;
    const checkTopicStr = `
        SELECT * FROM topics
        WHERE slug = $1`;

    return db.query(checkTopicStr, [topicQueryValue])
      .then(({ rows }) => {
        if (rows.length === 0) {
          return Promise.reject({ status: 404, msg: "Not Found" });
        } else {
          ...
          return db.query(fetchArticlesStr, [topic])
            .then(({ rows }) => {
              return rows;
            });
          };
...
WHERE article_id = $1;
`;
  return db.query(fetchArtIdStr, [article_id]).then(({ rows }) => {
    if (rows.length > 0) {
      return rows[0];
    } else {
      return Promise.reject({ status: 404, msg: "Not Found" });
...
const fetchCommentByArticle = (article_id) => {
  ...
  const doesArticleExistStr = `
    ...
    `;
  return db
    .query(doesArticleExistStr, [article_id])
    .then(({ rows }) => {
      if (rows.length === 0) {
        return Promise.reject({ status: 404, msg: "Not Found" });
      }
    ...
    .then(() => {
      const comByArtStr = `
        ...
        WHERE article_id = $1;
        `;
      return db.query(comByArtStr, [article_id]).then(({ rows }) => {
        return rows;
      });
    });
};
...
const createComment = (article_id, sentComment) => {
  ...
  return db
    ...
    `;
      return db
        .query(doesUserExistStr, [usernameData])
        ...
        RETURNING *;`;
          ...
          return db.query(createComStr, commentData).then(({ rows }) => {
            return rows;
          });
        });
    });
